...
Secrets necessários em .streamlit/secrets.toml (ou no painel do Streamlit Cloud):
...
# Credenciais Google lidas como JSON string do secrets
_creds_raw = st.secrets["GOOGLE_CREDENTIALS_JSON"]
GOOGLE_CREDENTIALS_DICT = json.loads(_creds_raw) if isinstance(_creds_raw, str) else dict(_creds_raw)
...
def conectar_sheets():
    """Conecta ao Google Sheets usando conta de serviço via secrets."""
    scopes = [
...
if criticos_df.empty:
    ...
    if not EMAIL_SENHA:
        st.info("🔑 Configure a `EMAIL_SENHA` nos secrets do Streamlit para habilitar o envio automático.")
    else:
